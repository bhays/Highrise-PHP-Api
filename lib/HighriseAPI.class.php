...
class HighriseCustomField
{
	...
	{
		$xml  = "<subject-data>\n";
		$xml .= '<subject_field_id type="integer">' . $this->getId() . "</subject_field_id>\n";
...
class HighrisePerson extends HighriseAPI
{
	...
	public function toXML($with_id = true)
	{
		...
		if( is_array($this->custom_fields) )
		{
			...
			foreach( $this->custom_fields as $items ) 
			{
				$xml[] = $items->toXML();
			}
			$xml[] = "</subject_datas>";
	...
	public function addCustomField($field_id, $value)
	{
		$item = new HighriseCustomField();
		$item->setId($field_id);
		$item->setValue($value);
